...
const CrearFactura = () => {
  ...
  const apiUrl =
    "https://facturasapi202307161115.azurewebsites.net/api/productos";
  ...
  const actualizarProductos = async () => {
    try {
      const response = await axios.get(
        "https://facturasapi202307161115.azurewebsites.net/guardarproductos"
      );
  ...
  const agregarDetalle = () => {
    const productoSeleccionado = productos.find(
      (p) => p.pro_id === parseInt(producto)
    );

    if (
      productoSeleccionado &&
      cantidad > 0 &&
      cantidad <= productoSeleccionado.pro_stock
    ) {
      const detalleExistente = detalles.find(
        (detalle) => detalle.IdProducto === producto
      );
      if (detalleExistente) {
          ...
          const newDetalles = detalles.map((detalle) => {
            if (detalle.IdProducto === producto) {
              return {
                ...
                Subtotal: calcularSubtotalDetalle(
                  cantidadTotal,
                  productoSeleccionado.pro_pvp,
                  productoSeleccionado.pro_valor_iva
                ),
              };
            }
            return detalle;
      ...
      alert(
        "La cantidad ingresada no es válida o supera el stock disponible del producto."
      );
    }
  };
          ...
          {productos
            ...
            .map((producto) => (
              <option key={producto.pro_id} value={producto.pro_id}>
                {producto.pro_nombre}   - STOCK:   {producto.pro_stock}
              </option>
            ))}
        </select>
